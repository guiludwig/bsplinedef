likelihoodTargetTime <- function(theta,
                             ...
                             M = model0, X = x, Y = y, TIM = tim, L = lambda){
  ...
  dB2 <- splineDesign(knots = c(rep(attr(B$B2, "Boundary.knots")[1],
                                ...
                                rep(attr(B$B2, "Boundary.knots")[2],
                                    attr(B$B2, "degree")+1)),
                      #!# x = rep(X[, 2], m), outer.ok = TRUE,
                      x = X[, 2], outer.ok = TRUE,
                      derivs = 1)
  dWdx1 <- matrix(0, n, DF1*DF2)
  #!# for(i in 1:n) dWdx1[i,] <- kronecker(dB1[i, ], predict(B$B2, rep(X[, 2],m))[i, ])
  for(i in 1:n) dWdx1[i,] <- kronecker(dB1[i, ], predict(B$B2, X[, 2])[i, ])
  dWdx2 <- matrix(0, n, DF1*DF2)
  #!# for(i in 1:n) dWdx2[i,] <- kronecker(predict(B$B1, rep(X[, 1],m))[i, ], dB2[i, ])
  for(i in 1:n) dWdx2[i,] <- kronecker(predict(B$B1, X[, 1])[i, ], dB2[i, ])
  ...
  # lik <- 0 #!#
  # for(j in 1:m) lik <- lik -1*RFlikelihood(M, x = f1, y = f2, data = Y[TIM == TIM[j]])$loglikelihood #!#
  # lik <- -1*RFlikelihood(M, x = f1, y = f2, T = tim, data = Y)$loglikelihood + exp(L)*PEN
