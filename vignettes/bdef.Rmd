...
author: "Guilherme Ludwig, Ronaldo Dias"
date: "`r Sys.Date()`"
...
The function `bdef` finds the appropriate deformations $\hat{f}_1,$ $\hat{f}_2,$ as well as estimating the parameters for the spatial covariance function. The default covariance function is a Mat&eacute;rn covariance function plus nugget effect, with differentiability parameter $\nu = 2.5$ (fixed), unknown variance $\sigma_\eta^2$, unknown scale parameter $\gamma$ and unknown nugget effect $\sigma_\varepsilon^2.$ 

For example, fitting the default model to the simulated dataset:

```{r}
model.default <- bdef(x, y)
```

Estimated parameters can be recovered with

```{r}
model.default$theta1 # Spline coefficients for f_1
model.default$theta2 # Spline coefficients for f_2
model.default$model # Covariance model
```

The fitting process supports some customization. For example, from the previous output, we realize that the default covariance might be inappropriate. We may change it with the following command:

```{r}
G <- RMmatern(nu = 2.5, var = NA, scale = NA) # Per RandomFields 
# documentation, values left as NA are to be estimated
model.correct <- bdef(x, y, cov.model = G)
model.correct$model
```
...
```{r echo=FALSE, fig.align="center", message=FALSE, fig.width=6, fig.height=6}
plotGrid(model.default, margins = TRUE)
```
...
```{r}
try(plotGDdist(model.default)) # Fails
```
