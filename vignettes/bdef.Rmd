...
set.seed(1)
n <- 100
x1 <- seq(0.05,0.95, length.out = 10) # runif(n)
x2 <- seq(0.05,0.95, length.out = 10) # runif(n)
x <- as.matrix(expand.grid(x1, x2)) # cbind(x1,x2)
F1 <- function(x1, x2) {
  x1 
}
F2 <- function(x1, x2) {
...
plotGrid(list(window = list(x = range(x1), y = range(x2)),
              x = x, def.x = cbind(F1(x[,1],x[,2]), F2(x[,1],x[,2]))),
...
```{r eval=FALSE}
plotGrid(model.matern, margins = TRUE)
```
