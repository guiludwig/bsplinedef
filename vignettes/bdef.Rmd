...
The `bsplinedef` package depends on `RandomFields`, `nloptr` and `splines`, among others. It also uses `knitr` and `rmarkdown` to build the documentation.
...
model <- bdef(x, y, tim = 1:TIME, cov.model = covModelM,
              df1 = 6, df2 = 6)
```
...
```{r}
# Value set to $nu = 2.5$ 
G <- RMmatern(nu = 2.5, var = NA, scale = NA) + RMnugget(var = NA)
...
plot(model.matern$model, ylim = c(0,2), xlim = c(0,0.7),
     model = list(`true model` = RMexp(var = 1, scale = .25) + RMnugget(var = 1)))
```